...
public class BasicGoalsTest {
    ...
    public void enemyWithSpawner() throws IllegalArgumentException, InvalidActionException {
        DungeonManiaController dmc;
        dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame("d_basicGoalsTest_enemyWithSpawner", "c_basicGoalsTest_enemy");
        ...
        String ztsId = TestUtils.getEntitiesStream(res, "zombie_toast_spawner").findFirst().get().getId();
        res = dmc.interact(ztsId);
        assertEquals("", TestUtils.getGoals(res));
    }
}
