...
public class GameMap {
    ...
    public void init() {
        ...
        initRegisterLogicals();
    }

    private void initRegisterLogicals() {
        List<Wire> wires = getEntities(Wire.class);
        List<Switch> switchs = getEntities(Switch.class);
        List<LogicalEntity> logicEntities = getEntities(LogicalEntity.class);
        for (Wire w : wires) {
            for (Wire w2 : wires) {
                if (w != w2 && Position.isAdjacent(w.getPosition(), w2.getPosition())) {
                    w.subscribe(w2);
                }
            }
            for (LogicalEntity l : logicEntities) {
                if (Position.isAdjacent(w.getPosition(), l.getPosition())) {
                    w.subscribe(l);
                }
            }
    ...
    public Position dijkstraPathFind(Position src, Position dest, Entity entity) {
        ...
        Map<Position, Position> prev = dijkstraPath(src, dest, entity);
    ...
    public Map<Position, Position> dijkstraPath(Position src, Position dest, Entity entity)  {
    ...
    public int getDistanceBetweenNodes(Position src, Position dest, Entity entity) {
        Map<Position, Position> prev = dijkstraPath(src, dest, entity);
        Position current = dest;

        // If the destination is not reachable or there's no path, return a special value or throw an exception
        if (!prev.containsKey(current)) {
            return -1; // or throw new RuntimeException("No path found");
        }

        int distance = 0;
        while (prev.get(current) != null) {
            distance += nodes.containsKey(current) ? nodes.get(current).getWeight() : 1;
